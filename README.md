...
The agent name doubles as the Hermes **profile name**. Change it in `.setup-state.env`:
...
To use a different SOUL.md file, update `SOUL_PATH` in `.setup-state.env` and restart.
...
**WSL-detected IP is unreachable** — `hostname -I` in WSL returns the WSL virtual NIC address (`172.x.x.x`), not the Windows host address (`192.168.x.x`). If SSH connections fail, manually edit `SSH_HOST` in `.setup-state.env` to your Windows host's actual IP, then restart:
